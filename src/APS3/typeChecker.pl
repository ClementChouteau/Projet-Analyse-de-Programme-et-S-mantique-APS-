...
dec(G, fun(X, Tr, XsTs, prog(CS)), [(X, fleche(Ts, Tr))|G]) :-
	listeTypesArgs(XsTs, Ts),
	append(XsTs, G, XsTsG),
	cmds(XsTsG, CS, Tr).

dec(G, fun_rec(X, Tr, XsTs, prog(CS)), [(X, fleche(Ts, Tr))|G]) :-
	listeTypesArgs(XsTs, Ts),
	append(XsTs, G, XsTsG),
	cmds([(X, fleche(Ts, Tr))|XsTsG], CS, Tr).
...
stat(G, if_imp(E, prog(CS1), prog(CS2)), T) :-
	expr(G, E, bool), cmds(G, CS1, T), cmds(G, CS2, T).  % IF typé

stat(G, if_imp(E, prog(CS1), prog(CS2)), T1) :- % IF partiellement typé (1)
	expr(G, E, bool), cmds(G, CS1, void), cmds(G, CS2, T), simplifier(orVoid(T), T1).

stat(G, if_imp(E, prog(CS1), prog(CS2)), T1) :- % IF partiellement typé (2)
	expr(G, E, bool), cmds(G, CS1, T), cmds(G, CS2, void), simplifier(orVoid(T), T1).

stat(G, while(E, prog(CS)), orVoid(T)) :- expr(G, E, bool), cmds(G, CS, T).
